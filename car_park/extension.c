...
typedef enum {INVALID, NORMAL, SHOW, PRETTY, ANIMATE} Flags;
...
int findSolution(Cp cpArray[], Cp tried[], State* state, Flags flag);
...
void printCarpark(Cp carpark, Flags flag);
void printPath(Cp carpark, Cp cpArray[], Flags flag);
...
void solveCarpark(char* fileName, Flags flag) {
  ...
  int moves = findSolution(cpArray, tried, &state, flag);
  handleResult(moves);
}
...
int findSolution(Cp cpArray[], Cp tried[], State* state, Flags flag) {
  ...
  while ((state->nextCp < state->endArray) && (state->endArray < LARGEARRAY)) {
    ...
    if (isComplete(cpArray[state->nextCp])) {
      if ((flag == SHOW) || (flag == PRETTY)) {
        printPath(cpArray[state->nextCp], cpArray, flag);
      }
...
void printPath(Cp carpark, Cp cpArray[], Flags flag) {
  ...
  for (int i = 0; i < moves; i++) {
    printCarpark(cpArray[carpark.parents[i]], flag);
    for (int i = 0; i < ((carpark.width - 1) >> 1) ; i++) {
      printf(" ");
    }
    printf("\u2193\n");
  }
  printCarpark(carpark, flag);
  printf("\n");
}
